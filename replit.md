...
- **Sprint 36 sector templates**: `artifacts/nxttrack/supabase/sprint36_sector_templates.sql` voegt `sector_templates` (RLS: select=authenticated, write=platform_admin) + `tenants.sector_template_key` FK toe en seedt `football_school`/`swimming_school`/`generic`. Backfill zet alle bestaande tenants op `football_school` (zero-regression Houtrust). Per-tenant overrides leven onder `tenants.settings_json -> 'terminology_overrides'` (geen extra kolom). Resolver `getTenantTerminology(tenantId)` in `src/lib/terminology/resolver.ts` — `React.cache()`d, fallback-chain override→sector→generic→DEFAULT_TERMINOLOGY, mag nooit throwen. Provider gemount in `(tenant)/tenant/layout.tsx`; client-componenten gebruiken `useTerminology()`. Audit + 13-key mapping in `artifacts/nxttrack/docs/sector-templates-audit.md`. Out-of-scope: platform-admin UI, role-key renames, `members.player_type` opheffen.
