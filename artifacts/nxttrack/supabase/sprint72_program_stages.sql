-- ═════════════════════════════════════════════════════════════════
-- Sprint 72 — Program stages fundament (v0.29.0)
--
-- Vervangt vrije-tekst `groups.level_band` (Sprint 71) door een
-- gestructureerd model:
--
--   programs.use_stages bool      — opt-in per programma
--   program_stages                — naam/kleur/sort_order per programma
--   group_stages                  — many-to-many groep ↔ stage
--   intake_submissions
--     .recommended_stage_id       — automatisch afgeleid bij submit
--     .selected_stage_id          — handmatig vastgezet door admin
--
-- Backfill: bestaande `groups.level_band`-waarden worden per
-- (tenant, program) als stage opgevoerd en aan groepen gekoppeld.
-- Daarna wordt `groups.level_band` gedropt.
--
-- RPC `score_placement_candidates` wordt herbouwd: leest
-- `selected_stage_id` ▸ `recommended_stage_id` ▸ fallback naar
-- `preferences_json.preferred_level` (case-insensitive name-match
-- binnen het programma) en scoort 100 wanneer een kandidaat-groep
-- via `group_stages` aan diezelfde stage gekoppeld is.
--
-- Idempotent: alle DDL via `if not exists` / drop+create.
-- ═════════════════════════════════════════════════════════════════

-- ── (1) programs.use_stages ────────────────────────────────────
alter table public.programs
  add column if not exists use_stages boolean not null default false;

comment on column public.programs.use_stages is
  'Sprint 72: wanneer true, gebruikt placement-RPC stage-match i.p.v. '
  'overgeslagen level_match. Default false zodat bestaande tenants '
  'niet stuk gaan vóór de seed.';
...
create table if not exists public.program_stages (
  ...
  constraint program_stages_tenant_id_fkey
    foreign key (tenant_id) references public.tenants(id) on delete cascade,
  constraint program_stages_program_tenant_fk
    foreign key (program_id, tenant_id)
    references public.programs(id, tenant_id) on delete cascade,
  constraint program_stages_id_tenant_uq unique (id, tenant_id),
  constraint program_stages_name_program_uq unique (program_id, name),
  constraint program_stages_color_chk
    check (color is null or color ~* '^#[0-9a-f]{6}$'),
  constraint program_stages_name_len_chk
    check (char_length(trim(name)) between 1 and 64)
);

create index if not exists program_stages_tenant_program_idx
  on public.program_stages (tenant_id, program_id, sort_order);
create index if not exists program_stages_active_idx
  on public.program_stages (tenant_id, program_id) where archived_at is null;

alter table public.program_stages enable row level security;
drop policy if exists program_stages_tenant_all on public.program_stages;
create policy program_stages_tenant_all on public.program_stages
  using (public.has_tenant_access(tenant_id))
  with check (public.has_tenant_access(tenant_id));

drop trigger if exists program_stages_updated_at on public.program_stages;
create trigger program_stages_updated_at
  before update on public.program_stages
  for each row execute function public.handle_updated_at();

comment on table public.program_stages is
  'Sprint 72: stage-laag per programma (bv. zwemschool-badjes, '
  'judo-banden, voetbal-leeftijdscategorieën). Vervangt vrije-tekst '
  'groups.level_band.';

-- ── (3) group_stages (M:N) ─────────────────────────────────────
create table if not exists public.group_stages (
  group_id   uuid not null,
  stage_id   uuid not null,
  tenant_id  uuid not null,
  created_at timestamptz not null default now(),
  primary key (group_id, stage_id),
  constraint group_stages_tenant_id_fkey
    foreign key (tenant_id) references public.tenants(id) on delete cascade,
  constraint group_stages_group_tenant_fk
    foreign key (group_id, tenant_id)
    references public.groups(id, tenant_id) on delete cascade,
  constraint group_stages_stage_tenant_fk
    foreign key (stage_id, tenant_id)
    references public.program_stages(id, tenant_id) on delete cascade
);

create index if not exists group_stages_stage_idx
  on public.group_stages (stage_id);
create index if not exists group_stages_tenant_group_idx
  on public.group_stages (tenant_id, group_id);

alter table public.group_stages enable row level security;
drop policy if exists group_stages_tenant_all on public.group_stages;
create policy group_stages_tenant_all on public.group_stages
  using (public.has_tenant_access(tenant_id))
  with check (public.has_tenant_access(tenant_id));

comment on table public.group_stages is
  'Sprint 72: koppelt groepen aan een of meerdere stages binnen een '
  'programma. Door RPC `score_placement_candidates` gebruikt voor '
  'level_match.';

-- ── (4) intake_submissions: recommended/selected_stage_id ─────
alter table public.intake_submissions
  add column if not exists recommended_stage_id uuid,
  add column if not exists selected_stage_id    uuid;

do $$ begin
  alter table public.intake_submissions
    add constraint intake_submissions_recommended_stage_fk
    foreign key (recommended_stage_id, tenant_id)
    references public.program_stages(id, tenant_id) on delete set null;
exception when duplicate_object then null; end $$;

do $$ begin
  alter table public.intake_submissions
    add constraint intake_submissions_selected_stage_fk
    foreign key (selected_stage_id, tenant_id)
    references public.program_stages(id, tenant_id) on delete set null;
exception when duplicate_object then null; end $$;

create index if not exists intake_submissions_recommended_stage_idx
  on public.intake_submissions (recommended_stage_id)
  where recommended_stage_id is not null;
create index if not exists intake_submissions_selected_stage_idx
  on public.intake_submissions (selected_stage_id)
  where selected_stage_id is not null;

comment on column public.intake_submissions.recommended_stage_id is
  'Sprint 72: automatisch afgeleid bij submit op basis van '
  'preferences_json.preferred_level (case-insensitive name-match '
  'binnen het programma).';
comment on column public.intake_submissions.selected_stage_id is
  'Sprint 72: door tenant-admin gekozen stage. Heeft voorrang boven '
  'recommended_stage_id in placement-scoring.';

-- ── (5) Backfill `groups.level_band` → stages + koppelingen ────
-- Idempotent: gebruikt `on conflict do nothing` op de natuurlijke
-- (program_id, name) unique. Skipt netjes wanneer kolom al gedropt is.
do $$
declare
  v_has_col boolean;
  v_rec record;
begin
  select exists (
    ...
    where table_schema = 'public'
      ...
      and column_name  = 'level_band'
  ) into v_has_col;
  if not v_has_col then return; end if;

  -- 5a. Maak stages aan voor elke (tenant, program, level_band)-combi.
  for v_rec in
    select distinct g.tenant_id, g.program_id, trim(g.level_band) as level_band
    from public.groups g
    where g.level_band is not null
      and trim(g.level_band) <> ''
      and g.program_id is not null
  loop
    insert into public.program_stages (tenant_id, program_id, name, sort_order)
    values (v_rec.tenant_id, v_rec.program_id, v_rec.level_band, 100)
    on conflict (program_id, name) do nothing;
  end loop;

  -- 5b. Koppel groepen aan de zojuist (of eerder) aangemaakte stage.
  insert into public.group_stages (group_id, stage_id, tenant_id)
  select g.id, ps.id, g.tenant_id
  from public.groups g
  join public.program_stages ps
    on ps.tenant_id  = g.tenant_id
   and ps.program_id = g.program_id
   and lower(trim(ps.name)) = lower(trim(g.level_band))
  where g.level_band is not null
    and trim(g.level_band) <> ''
    and g.program_id is not null
  on conflict (group_id, stage_id) do nothing;

  -- 5c. Zet `use_stages=true` op programma's die nu minstens 1 stage hebben.
  update public.programs p set use_stages = true
  where exists (
    select 1 from public.program_stages ps where ps.program_id = p.id
  )
  and use_stages = false;
end $$;

-- ── (6) Drop level_band ─────────────────────────────────────────
alter table public.groups drop column if exists level_band;

-- ── (7) RPC `score_placement_candidates` — stage-aware ────────
...
begin
  ...
  -- Programma-level opt-in voor stage-scoring.
  if v_program is not null then
    select use_stages into v_program_use_stages
    from public.programs where id = v_program and tenant_id = v_tenant;
  end if;

  -- Target-stage afleiden: selected ▸ recommended ▸ naam-lookup.
  v_target_stage := coalesce(v_selected_stage, v_recommended_stage);
  if v_target_stage is null
     and v_program is not null
     and v_preferred_level is not null then
    select id into v_target_stage
    from public.program_stages
    where tenant_id  = v_tenant
      and program_id = v_program
      and archived_at is null
      and lower(trim(name)) = v_preferred_level
    limit 1;
  end if;

  if v_target_stage is not null then
    select name into v_target_stage_name
  ...
  group_stage_names as (
    ...
    join public.program_stages ps
      on ps.id = gs.stage_id and ps.tenant_id = gs.tenant_id
  ...
  scored as (
    select
      ...
      -- Sprint 72 — stage-match. Levert 0 wanneer programma
      -- `use_stages=false` of wanneer geen target_stage afgeleid kon
      -- worden (geen voorkeur, geen lookup-match). Rationale licht toe.
      case
        when not v_program_use_stages then 0
  ...
  select
    ...
    jsonb_build_object(
      ...
      'level',
        case
          when not v_program_use_stages then 'stages niet ingeschakeld voor dit programma'
          ...
          else 'voorkeur ' || coalesce(v_target_stage_name, '?') || ', groep heeft ' || array_to_string(s.stage_names, ', ')
        end,
...
comment on function public.score_placement_candidates(uuid) is
  'Sprint 72: stage-aware placement scoring. level_match leest '
  'selected_stage_id ▸ recommended_stage_id ▸ name-lookup van '
  'preferences_json.preferred_level binnen het programma. Scoort 100 '
  'wanneer kandidaat-groep via group_stages aan die stage gekoppeld is.';
