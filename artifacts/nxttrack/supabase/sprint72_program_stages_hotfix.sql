-- ═════════════════════════════════════════════════════════════════
-- Sprint 72 — Hotfix: program-consistency voor stage-koppelingen
--
-- Architect-bevinding: `group_stages` en
-- `intake_submissions.recommended_stage_id/selected_stage_id`
-- valideren alleen op tenant, niet op program. Hierdoor kunnen
-- (geforceerde) requests cross-program stage-koppelingen aanmaken
-- die `score_placement_candidates` corrupte level-scores opleveren.
--
-- Deze hotfix voegt DB-niveau invarianten toe via triggers en
-- patcht de RPC zodat een mismatched target-stage genegeerd wordt.
-- ═════════════════════════════════════════════════════════════════

-- ── (1) group_stages: program-match-trigger ───────────────────
create or replace function public._group_stages_check_program()
returns trigger
language plpgsql
as $$
declare
...
begin
  select program_id into v_group_program
  from public.groups where id = new.group_id and tenant_id = new.tenant_id;
  select program_id into v_stage_program
  from public.program_stages where id = new.stage_id and tenant_id = new.tenant_id;

  if v_group_program is null then
    raise exception 'group % has no program_id; cannot attach stage', new.group_id
      using errcode = '23514';
  end if;
  if v_stage_program is null then
    raise exception 'stage % not found in tenant %', new.stage_id, new.tenant_id
      using errcode = '23514';
  end if;
  if v_group_program <> v_stage_program then
    raise exception 'stage % belongs to program % but group % belongs to program %',
      new.stage_id, v_stage_program, new.group_id, v_group_program
      using errcode = '23514';
  end if;
  return new;
end $$;

drop trigger if exists group_stages_check_program on public.group_stages;
create trigger group_stages_check_program
  before insert or update on public.group_stages
  for each row execute function public._group_stages_check_program();

-- ── (2) intake_submissions: stage-program-match-trigger ──────
create or replace function public._intake_submissions_check_stage_program()
returns trigger
language plpgsql
as $$
declare
  v_stage_program uuid;
begin
  if new.recommended_stage_id is not null then
    select program_id into v_stage_program
    from public.program_stages
    where id = new.recommended_stage_id and tenant_id = new.tenant_id;
    if v_stage_program is null then
      raise exception 'recommended_stage_id % not in tenant %',
        new.recommended_stage_id, new.tenant_id using errcode = '23514';
    end if;
    if new.program_id is not null and v_stage_program <> new.program_id then
      raise exception 'recommended_stage_id % belongs to program %, submission program is %',
        new.recommended_stage_id, v_stage_program, new.program_id
        using errcode = '23514';
    end if;
  end if;
  if new.selected_stage_id is not null then
    select program_id into v_stage_program
    from public.program_stages
    where id = new.selected_stage_id and tenant_id = new.tenant_id;
    if v_stage_program is null then
      raise exception 'selected_stage_id % not in tenant %',
        new.selected_stage_id, new.tenant_id using errcode = '23514';
    end if;
    if new.program_id is not null and v_stage_program <> new.program_id then
      raise exception 'selected_stage_id % belongs to program %, submission program is %',
        new.selected_stage_id, v_stage_program, new.program_id
        using errcode = '23514';
    end if;
  end if;
  return new;
...
drop trigger if exists intake_submissions_check_stage_program
  on public.intake_submissions;
create trigger intake_submissions_check_stage_program
  before insert or update of program_id, recommended_stage_id, selected_stage_id
  on public.intake_submissions
  for each row execute function public._intake_submissions_check_stage_program();

-- ── (3) RPC defensive guard ───────────────────────────────────
-- Patch `score_placement_candidates` zodat een selected/recommended
-- stage die NIET in het submission-programma zit wordt genegeerd
-- (target_stage := null) en de rationale dit toelicht.
...
begin
  ...
  -- Defensive: stage moet bij hetzelfde programma horen als de
  -- submission. Mismatch → target opheffen + flag voor rationale.
  if v_target_stage is not null then
    select program_id into v_target_stage_program
    from public.program_stages
    where id = v_target_stage and tenant_id = v_tenant;
    if v_program is not null
       and v_target_stage_program is not null
       and v_target_stage_program <> v_program then
      v_target_stage := null;
