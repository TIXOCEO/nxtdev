-- Sprint 82b — Public token-authorized scoring RPC.
--
-- Architect review (1e iteratie) markeerde dat de publieke /voorstellen-flow
-- `score_placement_candidates` aanriep met een **anonymous** Supabase-client,
-- terwijl die RPC alleen aan `authenticated` is granted én bovendien een
-- `has_tenant_access(v_tenant)` guard heeft. Resultaat: 42501 voor elke
-- token-aanvrager → de pagina degradeerde altijd naar "Tijdelijk geen
-- voorstellen beschikbaar". Deze migratie introduceert een **wrapper-RPC**
-- die het review-token-hash valideert i.p.v. tenant-membership, en daarna
-- exact dezelfde scoring-logica draait.
--
-- Veiligheid: caller moet zowel het `p_submission_id` als de geldige
-- `p_token_hash` weten — autoriteit is dus het token (sha256 van een 32-byte
-- random), niet `auth.uid()`. Het token wordt single-use ingevuld door de
-- TS-laag na een succesvolle keuze, dus replay is onmogelijk.
--
-- Idempotent: drop + create.

set search_path = public;

drop function if exists public.score_placement_candidates_public(uuid, text);

create or replace function public.score_placement_candidates_public(
...
begin
  if p_token_hash is null or length(p_token_hash) = 0 then
    raise exception 'review_token_required' using errcode = '42501';
  end if;
  ...
  select s.tenant_id, s.program_id, s.contact_date_of_birth,
         ...
         (s.review_token_hash = p_token_hash
            and s.review_token_expires_at is not null
            and s.review_token_expires_at > now())
    into v_tenant, v_program, v_dob, v_prefs,
         v_selected_stage, v_recommended_stage, v_token_ok
  from public.intake_submissions s
  where s.id = p_submission_id;

  if v_tenant is null then return; end if;
  if not v_token_ok then
    raise exception 'invalid_or_expired_review_token' using errcode = '42501';
  end if;
  ...
  select s.group_id, s.session_id,
    ...
    jsonb_build_object(
      ...
      'target_stage_id',    coalesce(to_jsonb(v_target_stage::text), 'null'::jsonb),
...
revoke all on function public.score_placement_candidates_public(uuid, text) from public;
grant execute on function public.score_placement_candidates_public(uuid, text) to anon, authenticated;

comment on function public.score_placement_candidates_public(uuid, text) is
  'Sprint 82b — token-authorized scoring voor publieke /voorstellen-flow. Vereist match op intake_submissions.review_token_hash + niet-verlopen TTL. Geen has_tenant_access guard — autoriteit is het token zelf.';
